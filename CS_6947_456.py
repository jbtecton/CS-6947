...
ds = BatchSource(name="CS_6947_456", batch_config=dsf)
user_entity = Entity(name="user_CS_6947_456", join_keys=[Field("user_id", String)])
...
def feature_view_a_CS_6947_456(input_table):
    return input_table[["user_id", "timestamp", "value"]]

feature_service_CS_6947_456 = FeatureService(
    name="feature_service_CS_6947_456",
    description="Testing NaN versus zero output in aggregations from offline versus online store",
    online_serving_enabled=True,
    features=[
        feature_view_a_CS_6947_456
    ],
)


@batch_feature_view(
    name="feature_view_b_CS_6947_456",
    mode="pyspark",
    sources=[ds],
    entities=[user_entity],
...
def feature_view_b_CS_6947_456(df):
